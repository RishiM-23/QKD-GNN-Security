...
def run_simulation_and_export_telemetry(timeline, channels, epochs=1000, output_file="sequence_telemetry_output.csv"):
    ...
    injects varying attacks (Blatant vs Stealthy), and exports the telemetry.
    """
    headers = [
        "Link_ID", "QBER", "Signal_Count", "Decoy_Count", 
        "Attacked_Flag", "Key_Loss", "Source_Node", "Dest_Node"
    ]
    ...
    with open(output_file, mode='w', newline='') as file:
        ...
        for epoch in range(epochs):
            ...
            for link_id, link_data in channels.items():
                ...
                # ---------------------------------------------------
                # THE ADVANCED ATTACK INJECTOR (WEEK 2)
                # ---------------------------------------------------
                # Increased to 10% chance an attacker targets this link
                if random.random() < 0.10:
                    is_attacked = 1
                    
                    # Flip a coin: 0 = Stealthy Attack, 1 = Blatant Attack
                    attack_type = random.randint(0, 1)
                    
                    if attack_type == 1:
                        # BLATANT ATTACK (Massive damage, easily caught by thresholds)
                        ...
                        # STEALTHY ATTACK (Hides in the noise, requires GNN to catch)
                        qber += random.uniform(0.02, 0.05) # Tiny QBER bump
                        signal_count = int(signal_count * random.uniform(0.85, 0.95)) # Barely drops
                        decoy_count = int(decoy_count * random.uniform(0.70, 0.85))
                        key_loss = random.uniform(0.1, 0.3) # 10% to 30% loss
...
if __name__ == "__main__":
    ...
    # Run Simulation Loop (Scaled up to 20,000 epochs for Week 2)
    run_simulation_and_export_telemetry(sim_timeline, network_channels, epochs=20000)